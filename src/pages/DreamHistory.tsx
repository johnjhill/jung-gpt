...
const extractTheme = (analysis: string): string => {
  // Try to extract a theme from the first sentence of analysis
  ...
  const words = firstSentence.split(' ')
    ...
    .slice(0, 7)
    .join(' ');
  return words.length > 0 ? words : 'Dream Theme';
};
...
const DreamHistory = () => {
  ...
  return (
    <div className="container max-w-4xl mx-auto py-12 px-4">
      <h1 className="text-4xl md:text-5xl font-serif text-foreground text-center mb-12">
        Your Dream Journey
      </h1>
      
      <div className="space-y-8">
        {dreams?.map((dream) => (
          <Card key={dream.id} className="p-6 bg-card shadow-lg">
            <div className="mb-4">
            ...
            {dream.analysis && (
              <p className="text-sm font-medium text-muted-foreground mb-4 italic">
                {extractTheme(dream.analysis.initialAnalysis)}
              </p>
            )}
            <div className="prose prose-sm">
              <h3 className="text-xl font-serif text-foreground mb-4">Dream</h3>
              <p className="text-foreground mb-4">{dream.dream_content}</p>
              
              {dream.analysis && (
                <div className="mt-4">
                  <h4 className="text-lg font-serif text-foreground mb-2">Initial Analysis</h4>
